...
from models.layers.region_loss import RegionLossV2
from cfg.cfg import *
# from dynamic_conv import DynamicConv2d
from models.layers.dynamic_conv import dynamic_conv2d
from models.layers.pooling import GlobalMaxPool2d
from models.layers.pooling import GlobalAvgPool2d
from models.layers.pooling import Split
import pdb
...
class Darknet(nn.Module):
    ...
    def forward(self, x, metax, mask, ids=None):
        ...
        # output shape: (Batch_size*(num_classes=15 for training or 20 for finetune),num_anchor*(5+num_classes=1),13,13)
        return x
    ...
    def create_network(self, blocks):
        ...
        for block in blocks:
            if block['type'] == 'net' or block['type'] == 'learnet':
                ...
                continue
            elif block['type'] == 'convolutional':   #convlution block
                conv_id = conv_id + 1
                ...
                models.append(model)
            elif block['type'] == 'maxpool':      #maxpool block
                ...
                out_filters.append(prev_filters)
                models.append(model)
            elif block['type'] == 'avgpool':   #avgpool block
                model = GlobalAvgPool2d()
                out_filters.append(prev_filters)
                models.append(model)
            elif block['type'] == 'softmax':   #softmax block
                model = nn.Softmax()
                out_filters.append(prev_filters)
                models.append(model)
            elif block['type'] == 'cost':
